function Assignments() {
  const assignments = [
    { id: 1, title: 'React Components', course: 'React Basics', dueDate: '2026-06-15', status: 'pending', marks: '--/100' },
    { id: 2, title: 'State Management', course: 'React Basics', dueDate: '2026-06-20', status: 'pending', marks: '--/100' },
    { id: 3, title: 'API Integration', course: 'Node.js Course', dueDate: '2026-06-10', status: 'submitted', marks: '85/100' },
    { id: 4, title: 'Database Design', course: 'MongoDB Basics', dueDate: '2026-06-05', status: 'graded', marks: '92/100' }
  ];
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
          ...
          <div className="bg-white p-4 rounded-xl shadow-sm">
            ...
            <p className="text-2xl font-bold text-primary">{assignments.filter(a => a.status === 'submitted').length}</p>
          </div>
          <div className="bg-white p-4 rounded-xl shadow-sm">
              ...
              <tbody>
                {assignments.map(a => (
                  <tr key={a.id} className="border-t border-border hover:bg-bg">
                    <td className="px-4 py-3 font-semibold text-textDark">{a.title}</td>
                    <td className="px-4 py-3 text-sm text-textLight">{a.course}</td>
                    <td className="px-4 py-3 text-sm text-textDark">{a.dueDate}</td>
                    <td className="px-4 py-3">
                      <span className={`text-xs font-semibold px-2 py-1 rounded ${getStatusColor(a.status)}`}>
                        {a.status}
                      </span>
                    </td>
                    <td className="px-4 py-3 font-semibold text-textDark">{a.marks}</td>
                    <td className="px-4 py-3">
                      {a.status === 'pending' && (
