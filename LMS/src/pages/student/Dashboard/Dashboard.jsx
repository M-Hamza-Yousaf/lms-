        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
            <div className="bg-white p-6 rounded-2xl shadow-lg">
              ...
              <h3 className="text-2xl font-bold text-textDark">{enrolledCourses.length}</h3>
          ...
          <div className="bg-white p-6 rounded-2xl shadow-lg mb-6">
            <div className="flex justify-between items-center mb-4">
              ...
              <Link to="/student/courses" className="text-primary text-sm font-semibold hover:underline">
                View All →
              </Link>
            </div>
            {enrolledCourses.length === 0 ? (
              <p className="text-textLight text-center py-8">No enrolled courses yet. Browse courses to get started!</p>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {enrolledCourses.slice(0, 3).map(course => (
                  <div key={course._id} className="border border-border p-4 rounded-lg hover:shadow-md transition">
                    <h3 className="font-bold text-textDark mb-1">{course.title}</h3>
                    <p className="text-xs text-textLight mb-3">{course.teacherName}</p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <Link to="/student/courses" className="bg-white p-6 rounded-2xl shadow-lg hover:shadow-xl transition">
              <div className="text-4xl mb-2">🎓</div>
              <h3 className="font-bold text-textDark">Browse Courses</h3>
              <p className="text-sm text-textLight">Discover new courses to learn</p>
            </Link>
            <Link to="/student/assignments" className="bg-white p-6 rounded-2xl shadow-lg hover:shadow-xl transition">
              <div className="text-4xl mb-2">📝</div>
