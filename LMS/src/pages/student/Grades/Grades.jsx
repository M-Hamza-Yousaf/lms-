function Grades() {
  const grades = [
    { course: 'React Basics', assignment: 'Components', marks: 85, total: 100, grade: 'A' },
    { course: 'React Basics', assignment: 'State Management', marks: 92, total: 100, grade: 'A+' },
    { course: 'Node.js Course', assignment: 'API Integration', marks: 78, total: 100, grade: 'B+' },
    { course: 'MongoDB Basics', assignment: 'Database Design', marks: 88, total: 100, grade: 'A' }
  ];
              ...
              <tbody>
                {grades.map((g, i) => (
                  <tr key={i} className="border-t border-border hover:bg-bg">
                    <td className="px-4 py-3 font-semibold text-textDark">{g.course}</td>
                    <td className="px-4 py-3 text-sm text-textDark">{g.assignment}</td>
                    <td className="px-4 py-3 text-sm text-textDark">{g.marks}/{g.total}</td>
                    <td className="px-4 py-3 text-sm text-textDark">{((g.marks/g.total)*100).toFixed(1)}%</td>
                    <td className="px-4 py-3">
                      <span className={`text-xs font-bold px-2 py-1 rounded ${getGradeColor(g.grade)}`}>
                        {g.grade}
                      </span>
                    </td>
                  </tr>
